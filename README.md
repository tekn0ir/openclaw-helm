...
Open Notebook runs from the single-container `lfnovo/open_notebook` image (bundled SurrealDB) and uses the local llama.cpp server through its OpenAI-compatible API, so no external API keys are required. Its web UI is exposed on a fixed NodePort (`30502`). Configuration lives under `openNotebook` in `values.yaml`, including the public `API_URL`/CORS origin used when the service is accessed through Istio and Keycloak.
